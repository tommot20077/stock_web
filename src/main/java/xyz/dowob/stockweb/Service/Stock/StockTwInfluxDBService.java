...
public class StockTwInfluxDBService {
    ...
    public void writeStockTwToInflux(JsonNode msgArray) {
        ...
        for (JsonNode msgNode : msgArray) {
            ...
            if (Objects.equals(msgNode.path("z").asText(), "--") || Objects.equals(msgNode.path("h").asText(), "--") || Objects.equals(msgNode.path("o").asText(), "--") || Objects.equals(msgNode.path("l").asText(), "--")) {
                continue;
            }
    ...
    private void writeKlineDataPoint(Long todayTlong, String stockCode, String tradeVolume, String openingPrice, String highestPrice, String lowestPrice, String closingPrice) {
        ...
        Point point = Point.measurement("kline_data")
                           ...
                           .addField("high", (new BigDecimal(highestPrice)).divide(twdToUsd, 3, RoundingMode.HALF_UP).doubleValue())
                           .addField("low",  (new BigDecimal(lowestPrice)).divide(twdToUsd, 3, RoundingMode.HALF_UP).doubleValue())
                           .addField("open",  (new BigDecimal(openingPrice)).divide(twdToUsd, 3, RoundingMode.HALF_UP).doubleValue())
                           .addField("close", (new BigDecimal(closingPrice)).divide(twdToUsd, 3, RoundingMode.HALF_UP).doubleValue())
